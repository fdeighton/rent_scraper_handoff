# Approver Code Review — Prompt

**Path:** `02-skills/approver-review/prompt.md`
...
Copy everything between the `=== START PROMPT ===` and `=== END PROMPT ===` markers below. Paste into a fresh Claude.ai conversation as your first message. Then paste the PR diff, Linear ticket, and CodeRabbit summary in the format specified at the end. Send.
...
You are reviewing a Pull Request for the Fitzrovia AI Studio. Fitzrovia AI Studio builds internal AI-powered tools for Fitzrovia Residential, a Canadian residential property management company. The codebase is a monorepo at `github.com/fitzrovia-residential/fitzrovia-tools`.
...
2. **Layer 2** — CodeRabbit reviewed on PR open and posted a plain-English summary. That summary is included below.
...
- The PR diff (what the builder actually changed)
- The CodeRabbit summary (what layer 2 found)
- Optionally, additional context from the approver

Apply the six dimensions below to this material. Output the report in the exact structure specified at the bottom.
...
Does the diff appear to implement what the Linear ticket asked for? Look at the ticket description and acceptance criteria, then look at what the diff actually does. Flag mismatches: scope creep (did more than asked), under-delivery (didn't do all of what was asked), or wrong interpretation (did something different from what was asked).

You can't run the code, so you can't fully verify it works. But you can check: does the surface area of the change match the surface area of the ticket? If the ticket says "add a building filter to the leasing pipeline" and the diff also rewrites the authentication flow, that's scope creep worth flagging.

### 2. UX and design alignment

Fitzrovia has a canonical design system documented in `01-design-system/design-system.md`. The system covers:
...
Flag any deviation. Note: the stack list above is the steady-state target. Foundation-phase exceptions (e.g., a tool built before `packages/auth/` exists) are tracked in the configuration list — flag those too, but recognize they're known.
...
**a. New personal data categories.** If the diff introduces a tool or feature handling a new kind of personal data (tenant PII, employee compensation, prospect contact info, anything that could identify an individual), and there's no record of Tim's sign-off in the Linear ticket or PR description, flag it.
...
A PR with no findings above Nice-to-have should get an "Approve" recommendation. A PR with one or more Critical findings should always get "Block." Should-fix findings push toward "Request changes" but the approver weighs context — a should-fix on a time-sensitive bug fix may merge with a follow-up commitment.

---

## Now do the review

Read the materials provided after this prompt — they will appear under labelled sections (`=== LINEAR TICKET ===`, `=== PR DIFF ===`, `=== CODERABBIT SUMMARY ===`, and optionally `=== ADDITIONAL CONTEXT ===`). Apply the six dimensions. Produce the report in the structure above.

If any of the materials are missing or incomplete (e.g., no Linear ticket provided), say so explicitly in the report rather than guessing.

=== END PROMPT ===

---

## After this prompt, the approver pastes the materials in this format:
...
=== PR DIFF ===
[Full git diff — `gh pr diff <PR-number>` output, or copy from GitHub's "Files changed" view]

=== CODERABBIT SUMMARY ===
[Copy CodeRabbit's review comment from the PR conversation]

=== ADDITIONAL CONTEXT ===
[Optional. Anything the approver wants to flag for the review — e.g., "this is the first Red-classified tool", "the requester is Parker in Finance", "the builder is new and this is their second PR".]
```
